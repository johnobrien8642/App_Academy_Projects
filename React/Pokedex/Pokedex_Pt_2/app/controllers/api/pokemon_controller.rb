class Api::PokemonController < ApplicationController
  ...
  def create 
    ...
    if @pokemon.save
      ...
      if !Move.pluck(:name).any?{|name| name == move_params[:move_3]}
        move3 = Move.create!(name: move_params[:move_3])
        PokeMove.create!(pokemon_id: @pokemon.id, move_id: move3.id)
      else
        PokeMove.create!(pokemon_id: @pokemon.id, move_id: Move.find_by(name: move_params[:move_3]).id)
      end
      render :show
    else
  ...
  def update 
    ...
    if @pokemon.update_attributes(pokemon_params)
      poke_moves = PokeMove.where(pokemon_id: @pokemon.id)
      poke_moves.destroy_all
      if !Move.pluck(:name).any?{|name| name == move_params[:move_1]}
        move1 = Move.create!(name: move_params[:move_1])
        PokeMove.create!(pokemon_id: @pokemon.id, move_id: move1.id)
      else
  ...
  def move_params
    params.require(:pokemon).permit(:move_1, :move_2, :move_3)
  end
